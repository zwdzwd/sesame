...
#' @param continuous whether the data should be treated as continuous
#' or discrete
#' @param cmp a CMPar object, for tunning color mapping parameters
#' @param cm a given color map
#' @param xticklabels to plot xtick labels, one may supply characters
#' to plot just a subset of xtick labels
#' @param xticklabels.n number of xtick labels to plot (resample for
#' aethetics by default)
...
#' @param yticklabels to plot ytick labels, one may supply characters
#' to plot just a subset of ytick labels
#' @param yticklabels.n number of ytick labels to plot (resample for
#' aethetics by default)
...
SplitWHeatmap <- function(hm, dm, cm, group) {
    ...
    sub.dms.col <- column.split[order(
        sapply(column.split, function(dm) dm$left))]
    sub.dms.row <- rev(row.split[order(
        sapply(row.split, function(dm) dm$bottom))])
    ...
    k <- apply(sub.dms, 1, function(dm.i) {
        ...
        sub.hm <- hm
        sub.hm$dm <- WDim(
            sub.dm.col$left, sub.dm.row$bottom,
            sub.dm.col$width, sub.dm.row$height,
            row.split=sub.dm.row$row.split,
            column.split=sub.dm.col$column.split,
...
CalcTextBounding.WHeatmap <- function(hm, group) {
    ...
    if (!(is.null(hm[['yticklabels']]) || (
        length(hm[['yticklabels']])==1 && hm[['yticklabels']] == FALSE))) {
        ...
        if (hm$yticklabel.side=='l') {
            if (is.null(rownames(hm$data))) {
                ...
                .text.margin <- max(sapply(
                    rownames(hm$data),
                    function(t) text.width(t, hm$yticklabel.fontsize))) -
    ...
    if (!(is.null(hm[['xticklabels']]) || (
        length(hm[['xticklabels']])==1 && hm[['xticklabels']] == FALSE))) {
        if (hm$xticklabel.side=='b') {
            if (is.null(colnames(hm$data))) {
                ...
                .text.margin <- max(sapply(
                    colnames(hm$data),
                    function(t) text.width(t, hm$xticklabel.fontsize))) -
...
#' @return \code{NULL}
#' @import grid
print.WHeatmap <- function(
    x, cex=1, layout.only=FALSE, stand.alone=TRUE, ...) {
    ...
    grid.rect(
        xc[expand.index[[2]]], yc[expand.index[[1]]],
        width=unit(1/nc, 'npc'), height=unit(1/nr, 'npc'),
        gp=do.call('gpar', c(list(fill=x$cm$colors), x$gp)),
        just=c('left','bottom'))
