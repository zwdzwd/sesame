...
#' @param label2color a named vector or list that defines label to color
#' mapping
#' explicitly for discrete color mapping
#' @param brewer.name palette name for RColorbrewer
#' @param brewer.n number of stop points in RColorBrewer for continuous
#' color map
...
MapToContinuousColors <- function(data, cmp=CMPar(), given.cm=NULL) {
    ...
    if (is.null(cmp$stop.points)) {
        ...
        if (!is.null(cmp$cmap)) {
            ...
            ## diverge_hcl, diverge_hsv, terrain_hcl, heat_hcl,
            ## sequential_hcl and rainbow_hcl
