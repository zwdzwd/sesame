...
WDendrogram <- function(
    clust=NULL, dm=WDim(0,0,1,1), name='',
...
print.WDendrogram <- function(
    x, stand.alone=TRUE, layout.only=FALSE, cex=1, ...) {
...
#'
#' The dendrogram can be renderred. A viewport is created which
#' contains the dendrogram.
#'
#' -order should leaves of dendrogram be put in the normal order
#' (1, ..., n) or reverse order (n, ..., 1)?
#' -... pass to `grid::viewport` which contains the dendrogram.
#'
#'
#' This function only plots the dendrogram without adding labels.
#' The leaves of the dendrogram
...
#' @import grid
#' @return view port that contains dendrogram
grid.dendrogram <- function(
    ...
    draw.d <- function(
        dend, max_height, facing = "bottom", order = "normal",
        max_width = 0, env = NULL) {
        ...
        for(gp_name in c("col", "lwd", "lty")) {
            if(is.null(edge_gp1[[gp_name]])) {
                env[[gp_name]] <- c(
                    env[[gp_name]], rep(get.gpar(gp_name)[[gp_name]], 2))
            } else {
                env[[gp_name]] <- c(
                    env[[gp_name]], rep(edge_gp1[[gp_name]], 2))
            }
            if(is.null(edge_gp2[[gp_name]])) {
                ...
                env[[gp_name]] <- c(
                    env[[gp_name]], rep(edge_gp2[[gp_name]], 2))
            }
        }
        ...
        if(order == "normal") {
            if(facing == "bottom") {
                ...
                env$x1 <- c(env$x1, c(x1, (x1+x2)/2, x2, (x1+x2)/2))
                env$y1 <- c(
                    env$y1, max_height - c(height, height, height, height))
            } else if(facing == "right") {
                env$x0 <- c(env$x0, max_height - c(y1, height, y2, height))
                env$y0 <- c(env$y0, max_width - c(x1, x1, x2, x2))
                env$x1 <- c(
                    env$x1, max_height - c(height, height, height, height))
                env$y1 <- c(
                    env$y1, max_width - c(x1, (x1+x2)/2, x2, (x1+x2)/2))
            } else if(facing == "left") {
            ...
            if(facing == "bottom") {
                ...
                env$y0 <- c(env$y0, c(y1, height, y2, height))
                env$x1 <- c(
                    env$x1, max_width - c(x1, (x1+x2)/2, x2, (x1+x2)/2))
                env$y1 <- c(env$y1, c(height, height, height, height))
            } else if(facing == "top") {
                env$x0 <- c(env$x0, max_width - c(x1, x1, x2, x2))
                env$y0 <- c(env$y0, max_height - c(y1, height, y2, height))
        ...
        if(begin) {
            grid.segments(
                env$x0, env$y0, env$x1, env$y1,
                default.units = "native",
                gp = gpar(col = env$col, lty = env$lty, lwd = env$lwd))
        }
    }

    labels <- as.character(labels(dend))
    ## leaves are places at x = 0.5, 1.5, ..., n - 0.5
