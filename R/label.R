...
WLabel <- function(
    x=NULL, dm=WDim(), name='', fontsize=12, rot=0, color='black') {
...
print.WLabel <- function(x, cex=1, layout.only=FALSE, stand.alone=TRUE, ...) {
    ...
    if (!layout.only) {
        ...
        grid.text(
            x$x, x=unit(x$dm$text.x,'npc'),
            y=unit(x$dm$text.y, 'npc'),
            just=text.just,
            rot=x$rot,
            gp=gpar(fontsize=x$fontsize*cex, col=x$color))
    }
}
...
CalcTextBounding.WLabel <- function(label, group) {
    ...
    if ("call" %in% class(label$x)) {
        ## TODO: fix call language object,
        ## text.width/stringHeight doesn't work
        width <- 0
        height <- 0
    } else {
