...
inferSpecies <- function(sdf, df_as = NULL, topN = 3000,
  ...
  if (success.rate >= threshold.sucess.rate && sdfPlatform(sdf) == 'MM285') {
    return(list(auc=1,species='Mus musculus',taxid='10090',success_rate=success.rate))
  }
  ...
  if (length(y_true) == 0){
    if (ret.max){
      return(list(auc=NA,species=NA,taxid=NA,success_rate=success.rate))
    } else {
      return(as.data.frame(t(vapply(colnames(df_as),function() {
        list(auc=NA,species=NA,taxid=NA,success_rate=success.rate)
      }, c(numeric(1), character(1), character(1),numeric(1))))))
    }
  }
  ...
  auc = vapply(colnames(df_as),function(s) {
    ...
    U1 = R1 - n1 * (n1 + 1)/2
    #U1 / (n1 * n2)   #Error: In n1 * n2 : NAs produced by integer overflow
    (U1/n1)/n2
  }, numeric(1))
