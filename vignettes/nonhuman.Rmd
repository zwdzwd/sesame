...
the `openSesame` function. See [The Basics Vignette](sesame.html#prep) for more
detail.
...
`return.pval=TRUE` or `return.probability=TRUE` to return the inferred
strain, the p-value or the probabilities of all 37 strain candidates.
...
always measured by the M-allele of the probe. SeSAMe flips the $\beta$ values for
some probes to calculate variant allele frequency. The following example shows
what `inferStrain` does to a `SigDF`:
...
Vignette](https://zhou-lab.github.io/sesame/supplemental.html#SNP) for heatmap
validation of strain inference.
...
elaborated in [The Inference Vignette](inference.html).
