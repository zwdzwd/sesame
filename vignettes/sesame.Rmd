...
documentation consistently. If you install directly from github, we should
make sure the compatible ExperimentHub is installed.

> **CRITICAL:** After a new installation, we need to cache the associated
annotation data using the following command.  This needs to be done only once
per SeSAMe installation/update.
...
levels ($\beta$ values, the `getBetas` function).
...
[below](#Customize_preprocessing) for detail) which `openSesame` calls
...
df <- data.frame(rbind(
   ...
   c("MMB", "mouse", "TQCDPB"),
   c("MMB", "non-mouse organism", "SQCDPB"),
...
organisms, mouse strains etc. Some platforms (e.g., Mammal40 and MMB) are
