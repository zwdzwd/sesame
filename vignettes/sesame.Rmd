...
The `openSesame` function is highly customizable. The `prep=` argument allows a
different preprocessing strategy (see the `prepSesame` function below for how
to use a different preprocessing procedure). The `func=` option specifies the
signal extraction function which can be either `getBetas` (DNA methylation) or
`getAFs` (allele frequencies of SNP probes). The `manifest=` option allows the
processing of a custom array that is not natively supported by providing the
array manifest. Finally, the `BPPARAM=` argument is the same argument taken by
...
a `SigDF` and returns a processed `SigDF`. The `prepSesame` function also takes
a string that encodes the preprocessing functions and the order of their
...
performs dye bias correction (D) followed by background subtraction
(B). `prepSesame(sdf, "DB")` is equivalent to `noob(dyeBiasNL(sdf))`. The
following table lists all the available codes and the associated preprocessing
functions.
...
All the preprocessing functions take a SigDF as input and output the updated
SigDF. Therefore they can be chained together (see the `Preprocessing
functions` section for a more detailed discussion of these functions). The
optimal strategy of preprocessing depends on 1) the array platform; and 2) the
expected sample input. For example, certain array platforms (such as the
Mammal40) do not contain enough Infinium-I probes for normalization, therefore
background subtraction might not work most optimally. Certain platform (e.g.,
Mammal40 and MM285) are designed for multiple species and strains. Therefore
`inferSpecies` and `inferStrain` would be of help. The following table lists
the recommended preprocessing strategy based on our experience. The order of
different preprocessing also matters.
