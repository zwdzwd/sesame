...
[sesameData](https://tinyurl.com/58ny3rrt) package which is based on the
[ExperimentHub](https://tinyurl.com/2p873tez) package.  You can find the
...
files for the same sample. `SigDF` can be treated as a regular data.frame:
...
We will get back to the structure of `SigDF` in detail below.
...
NA-masking). With the manifest, your data can be processed using

```{r eval = FALSE}
sdf = readIDATpair("your_sample_prefix", manifest = mft)
```

# Search IDAT Prefixes

Most often we will be working with a folder that contains many IDATs. Here is
where the `searchIDATprefixes` function comes in useful. It lets us search all
