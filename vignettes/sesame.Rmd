...
is the data structure named SignalSet, a R6 reference class with slots
containing the signals for six different classes of probes:
...
Current implementation based on SignalSet R6 is more efficient and reduces
data-copy overhead. Most functions include an option `in.place` which speeds 
up execution by avoiding additional data copy.
...
A simple list of "SignalSet"s are returned. One can also just provide a vector
of file paths prefixes (excluding `_Red.idat` and `_Grn.idat`, one prefix for
a pair of IDATs) and call `readIDATs` directly.
...
qqplot(
    sset.dbLinear$IR, sset.dbLinear$IG,
...
```{r}
qqplot(
    sset.dbNonlinear$IR, sset.dbNonlinear$IG,
    xlab='Type-I Red Signal', ylab='Type-I Grn Signal',
...
ssets.normal <- readRDS(system.file(
    "extdata", "EPIC.sset.cn.normals.rds", package = "sesameData"))
