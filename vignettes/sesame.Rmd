...
If you want to have more granuality of control, openSesame is equivalent to
```{r message = FALSE, warning = FALSE, eval = FALSE}
betas <- do.call(cbind, lapply(searchIDATprefixes(idat_dir), function(pfx) {
    pfx %>% readIDATpair %>% pOOBAH %>%
        noob %>% dyeBiasCorrTypeINorm %>% getBetas
}))
```
behind the scene.
...
threshold can be adjusted to say, 0.1, by `pval.threshold = 0.1` in either
`getBetas` or `openSesame`. 
...
only supports EPIC, HM450 and HM27 and is turned on by default.  It can be
turned off by setting `quality.mask = FALSE` for either `getBetas` or
`openSesame`.
