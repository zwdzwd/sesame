...
```{r ky10, run-test-single, echo=TRUE, eval=TRUE}
df <- rowData(sesameDataGet('MM285.tissueSignature'))
...
```{r ky13, fig.width=6, fig.height=4, warning=FALSE}
query <- df$Probe_ID[df$branch == "fetal_brain" & df$type == "Hypo"]
...
```{r ky16, fig.width=7, fig.height=6, echo=TRUE, warning=FALSE}
...
```{r ky20, plot-meta1, fig.width=5, fig.height=4, eval=FALSE}
...
```{r ky21, run-test-data, echo=TRUE, eval=TRUE}
...
``` {r ky24, run-feature-engineering-get-data, echo=TRUE, eval=TRUE}
library(SummarizedExperiment)
...
```{r ky25, run-feature-engineering-statistics}
