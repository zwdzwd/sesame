...
```{r figs, echo=FALSE, fig.cap="Two highly correlated CpGs belonging to the same module. Across 256 samples from different tissues, ages and sexes, methylation changes at one CpG is accompanied by similar changes in a separate co-methylated CpG"}

se <- sesameDataGet("MM285.10.SE.tissue")


## data <- readRDS("/Users/goldbergd2/20210319_1656_mouse_array_samples.rds")
## tissues <- c("Cecum", "Colon", "Small Intestine")
## sampleSheet <- read_xlsx("/Users/goldbergd2/Desktop/Lab/20210713_MouseArray_SampleTableFinal_local.xlsx")
## IDs <- sampleSheet[sampleSheet$Tissue %in% tissues & sampleSheet$Strain == "C57BL/6J", ]$IDAT
## mod_cpgs <- readRDS(file="/Users/goldbergd2/Desktop/Lab/20220407_KYCG_WMCNA_NND_Results.RData")
## sub <- as.data.frame(data[mod_cpgs$CpGs[[1]], IDs]) #pick 2 random CpGs to plot and show correlation across samples
## df_plot <- as.data.frame(t(sub[c("cg30910045_BC21","cg34861418_TC21"),])) %>% rownames_to_column()

df_plot <- as.data.frame(t(assay(se)[c("cg30910045_BC21","cg34861418_TC21"),])) %>% rownames_to_column("IDAT")
