...
**Choice of universal set:** Universal set is the set of all probes for a given
...
these are supplied, the universe set will be implied from the probes.
...
The estimate here is enrichment score. **Negative enrichment score suggests
enrichment of the categorical database with the higher values (in the numerical
database). Positive enrichment score represent enrichment with the smaller
values**. As expected, the designed TSS CpGs are significantly enriched in
smaller TSS distance and higher CpG density.
