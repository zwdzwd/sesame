...
df <- rowData(sesameDataGet('MM285.tissueSignature'))
query <- df$Probe_ID[df$branch == "fetal_liver" & df$type == "Hypo"]
results <- testEnrichmentGene(query, platform="MM285")
head(results)
```
...
testEnrichment(query, "MM285.seqContextN")
```

The estimate here is enrichment score. Negative enrichment score suggest
enrichment with the higher values of the database. As expected, the designed
TSS CpGs are significantly enriched in smaller TSS distance and higher CpG
density.
