...
See [Supplemental
Vignette](https://zhou-lab.github.io/sesame/v1.18/supplemental.html#kycg_vis)
...
[here](http://zwdzwd.github.io/InfiniumAnnotation#KYCG). You can use a 
convenience function to download these database sets to a local folder.

```{r eval=FALSE}
KYCG_downloadDBs("EPICv2", "~/Downloads/test")
KYCG_listDBGroups(path = "~/Downloads/test")
...
dbs <- KYCG_loadDBs("~/Downloads/test/") # all database files in the folder
## or
dbs <- KYCG_loadDBs("~/Downloads/test/CGI.20220904.gz") # one database file
```
