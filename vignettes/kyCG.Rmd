...
plotVolcano(resultsTFBS, alpha=0.0005)
```

```{r plot-lollipop-tfbs, fig.width=7, fig.height=6, echo=TRUE}
plotLollipop(resultsTFBS)
```
...
We will calculate the summary statistics for the betas matrix using a list of database sets. The default is to calculate the mean.
...
model = lm(Mouse_Age_Days ~ Sex + Strain_Corrected + Tissue_Corrected + 
    Genotype + CGI + CTCF + Clock + SpermMeth + VMR, data=data)
```
