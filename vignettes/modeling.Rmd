...
Here we demonstrate the analysis of DNA methylation dependent on one or
multiple predictors. The predictors might be it tissue, cell type, sex, age,
tumor/normal, treatment/control or a combination of these factors.  The `DML`
(short for Differential Methylation Locus) function models beta values (DNA
methylation levels) on known co-variates (e.g., case vs control) using linear
modeling. This general supervised learning framework identifies CpG loci whose
differential methylation is associated with known co-variates. Let's first load
needed packages
...
se = sesameDataGet("MM285.10.SE.tissue")[1:1000,] # an arbitrary 1000 probes
colData(se)
```
...
* **Pval_**
: The **unadjusted** p-values of t-testing the slope (statistical
...
* **FPval_**
: The **unadjusted** p-value of the F-test contrasting the full model against a
reduced model with the labeled contrasting held out. Note that "Pval_" and
"FPval_" are equivalent when the contrast variable is a 2-level factor, i.e., a
pairwise comparison.
...
function. It is also integrated to the `DMR` function by default (see below).
...
See [Supplemental
Vignette](https://zhou-lab.github.io/sesame/supplemental.html) for track-view
