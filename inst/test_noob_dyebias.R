...
create.validation <- function() {
  ...
  map <- read.csv('samples.csv', stringsAsFactors=FALSE)
...
options(mc.cores=8)
ssets <- readIDATsFromSheet("../../test/data/tcga.random6/samples.csv", base.dir='../../test/data/tcga.random6', mc=TRUE)
ssets <- mclapply(ssets, noob)
ssets <- mclapply(ssets, dyeBiasCorr)
betas <- signalToBeta(ssets, na.mask=FALSE, mc=TRUE)
